...
Before we begin with the code I thought it would be good to explain my motivation for doing this analysis. Recently I won a [case](http://ustavensud.mk/?p=19176) at the Macedonian Constitutional Court. I won't go into the legal details here, but from the outset the challenged provision in the Law on execution (if this is an appropriate translation) seemed wrong i.e. unconsitutional.  When finally the Court agreed it was wrong, I asked my self, why no one else thought of this before, or even better why did the court wait for someone to appeal before it acts. After all the court has mandate to act on it own. and this was not a new law. So with this questions in my head I decided to investigate which court decisions are made after appeals from the public, and which are the courts own doing. Enter `R`.
...
Before moving to part two I thought I should explain some of my thinking for the above code.
I decided to scrape (and store) all the text from the court documents, although for the current analysis I probably won't use it all. 

I was struggling to figure out how to keep the column `text` in a nested list so I can easily access the elements I need -- see comment above in the code -- but as I couldn't find a way to do it, I settled with the tibble in long format. I can probably do `group_by("url") %>% nest` at some point if I need that. 

I also decided to store the tibbles in RDS format. I think I read somewhere that RDS is useful beacause of compression. And since I am pushing the code to Github I wanted to be on the save side. Also, I didn't want to rerun the scraping everytime I need to work on the text -- or better said, court decisions come at a rate of few in a month, and scraping daily as we are digging through the text makes little sense.

## Part II: Analyzing the Court's documents.

```{r, echo = FALSE}
odluki <- readRDS("data/odluki_na_ustaven_sud.RDS")
resenija <- readRDS("data/resenija_na_ustaven_sud.RDS")
```
